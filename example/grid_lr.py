...
def parse_args():
    p = argparse.ArgumentParser(description="Parallel LR sweep for example/nanogpt_train.py")
    p.add_argument("--lrs", type=float, nargs="+", required=True, help="Learning rates to sweep")
    ...
    p.add_argument("--run_name_prefix", type=str, default="owt_lr")
    return p.parse_args()
...
def build_cmd(args, lr, port):
    cmd = [
        ...
        "--start_port", str(port),
        "--run_name", f"{args.run_name_prefix}{lr:.0e}",
    ]
...
def main():
    ...
    pending = deque(enumerate(args.lrs))  # (idx, lr)
    running = {}  # gpu_id -> (Popen, log_file_path, file_handle)

    def start_on(gpu_id, lr_idx, lr):
        port = args.base_port + 10 * lr_idx  # stable unique port per run
        cmd = build_cmd(args, lr, port)
        env = os.environ.copy()
        env["CUDA_VISIBLE_DEVICES"] = str(gpu_id)
        env.setdefault("OMP_NUM_THREADS", "1")

        log_path = Path(args.logs_dir) / f"{args.run_name_prefix}{lr:.0e}_gpu{gpu_id}.log"
        fh = open(log_path, "w")
        print(f"[launch] gpu={gpu_id} lr={lr} port={port} -> {log_path}")
    ...
    for gpu_id in gpus:
        if pending:
            i, lr = pending.popleft()
            start_on(gpu_id, i, lr)
